...
This phase asymmetry forces serving systems to disaggregate or chunk requests~\cite{distserve2024}, creating modeling challenges that span both compute and memory subsystems; concurrently, multi-chip module (MCM) packaging, HBM3 bandwidth walls, and near-memory compute~\cite{upimulator2024,attacc2024,neupims2024} invalidate monolithic roofline assumptions, demanding new per-bank and die-to-die modeling abstractions.
The gap between hardware generations is widening: each new GPU family changes memory hierarchy depths, tensor-core scheduling policies, and on-chip interconnect topologies, making models calibrated on one generation unreliable for the next without architecture-aware transfer learning.
...
The rapid emergence of speculative decoding, retrieval-augmented generation, and multi-modal pipelines further strains existing tools, which assume fixed computation graphs and cannot model the input-dependent branching and variable-length retrieval stages these workloads introduce.
...
Standardizing layer-boundary representations would enable tool interoperability: a kernel predictor could feed per-operator latencies into a composition model, which in turn exports iteration-time estimates to a distributed training simulator, all through portable Chakra-compatible exchange formats.
Such a pipeline would also support continuous regression testing, where automated nightly runs compare simulator predictions against fresh hardware measurements, catching calibration drift before it propagates to downstream architectural decisions.
\begin{figure}[t]
\centering\resizebox{\columnwidth}{!}{%
...
\textbf{(3)~Hardware transfer:} Cross-family transfer (GPU$\rightarrow$TPU$\rightarrow$PIM~\cite{upimulator2024,attacc2024,neupims2024,paise2025}) and congestion modeling~\cite{astrasim2023,triosim2025,ns3_2010} remain unsolved; domain adaptation techniques that fine-tune learned models on small target-hardware datasets offer a path forward but require architecture-aware embedding spaces.
Concretely, Habitat's wave-scaling approach breaks down when moving from Ampere to Hopper due to new thread-block cluster scheduling, illustrating why hardware-specific recalibration layers are essential for cross-generation fidelity.
\textbf{(4)~Standardized evaluation:} No MLPerf~\cite{mlperf_training2020,mlperf_inference2020} equivalent exists for simulators; a community benchmark suite with portable formats~\cite{chakra2023}, reference measurements across GPU generations, and continuous validation~\cite{mlperfpower2025} is needed.
Such a suite should include at minimum: (a)~representative kernels from each model family (CNN, transformer, MoE, diffusion), (b)~end-to-end model traces with ground-truth latencies on at least three GPU generations, and (c)~distributed collective benchmarks spanning 2--128 nodes with realistic network topologies.
\textbf{(5)~Reproducibility:} Dependency rot is a practical barrier---nn-Meter failed due to incompatible libraries; containerized environments and nightly CI pipelines that rebuild toolchains and regression-test against golden traces are needed.
Beyond containers, reproducibility requires pinned dependency manifests, deterministic build hashes, and automated golden-trace comparison scripts that flag regressions exceeding a user-specified error tolerance across all supported hardware targets.
\section{Conclusion}\label{sec:conclusion}
We survey 22 ML performance tools and evaluate five against a 28-scenario benchmark, finding self-reported accuracy unreliable (NeuSight: 2.3\% claimed vs.\ 5.87--27.10\%; nn-Meter: no output). The 5--15\% kernel-to-model composition gap dominates total error; closing it requires validated composition models, standardized layer-boundary representations, open benchmark suites with ground-truth measurements, and community CI infrastructure for continuous validation across the rapidly diversifying hardware landscape.
Our evaluation artifacts---including benchmark scripts, configuration files, and reproducibility containers---are available to support future tool comparisons and community-driven validation efforts.
