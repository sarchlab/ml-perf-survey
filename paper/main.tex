...
Bridging these layers requires solving three integration challenges.
\emph{Interface heterogeneity:} each tool defines its own input format---Timeloop uses YAML workload descriptions, ASTRA-sim consumes Chakra execution traces~\cite{chakra2023}, and VIDUR replays tokenized request logs---so connecting Layer~1 output to Layer~4 input demands ad-hoc format conversion at every boundary.
\emph{Calibration mismatch:} analytical models assume steady-state execution, while real workloads exhibit transient effects (JIT compilation, cache warm-up, memory pool fragmentation) that dominate short-running kernels; a composition layer must reconcile these modeling assumptions.
\emph{Feedback loops:} serving systems adapt scheduling decisions (batching depth, preemption) based on predicted latency, creating circular dependencies that static pipelines cannot capture; closing this loop requires co-simulation across layers~3--5 rather than sequential hand-off.
\begin{figure}[t]
\centering\resizebox{\columnwidth}{!}{%
...
\textbf{(5)~Reproducibility and software maintenance:} Our evaluation reveals that dependency rot is a practical barrier---nn-Meter failed entirely due to incompatible library versions, and multiple tools required non-trivial patching to run on current hardware. Long-term impact requires that tools publish containerized environments, pin dependency versions, and maintain CI pipelines that validate against reference outputs. Without such infrastructure, even accurate tools become unusable within 2--3 years of publication.
