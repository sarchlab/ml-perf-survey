...
On the hardware side, multi-chip module (MCM) packaging composes accelerator dies, HBM3 stacks, and interconnect chiplets into heterogeneous packages, invalidating monolithic roofline assumptions; processing-in-memory architectures~\cite{upimulator2024,attacc2024,neupims2024} further couple compute throughput to memory access patterns, demanding per-bank modeling abstractions absent from current tools.
...
\textbf{(3)~Hardware transfer:} Cross-family transfer (GPU$\rightarrow$TPU$\rightarrow$PIM~\cite{upimulator2024,attacc2024,neupims2024,paise2025}) and congestion modeling~\cite{astrasim2023,triosim2025,ns3_2010} remain unsolved; domain adaptation---fine-tuning learned latency models on small target-hardware datasets while reusing source-hardware feature extractors---offers a promising direction but requires architecture-aware embedding spaces.
\textbf{(4)~Standardized evaluation:} No MLPerf~\cite{mlperf_training2020,mlperf_inference2020} equivalent exists for simulators; portable formats~\cite{chakra2023} and continuous validation~\cite{mlperfpower2025} are needed, ideally as a community benchmark suite spanning kernel, model, and system levels with reference measurements on multiple GPU generations.
\textbf{(5)~Reproducibility:} Dependency rot is a practical barrier---nn-Meter failed due to incompatible libraries; containerized environments and nightly CI pipelines that rebuild toolchains and run regression tests against golden traces are needed to catch breakage before it silently invalidates published results.
...
Realizing the unified five-layer pipeline requires coordinated investment in standardized layer-boundary representations, open benchmark suites with ground-truth measurements, and community CI infrastructure for continuous validation.
