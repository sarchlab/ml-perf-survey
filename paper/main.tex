...
\label{subsec:llm-benchmark-suite}

Rather than evaluating each tool on its own preferred workloads, we define a benchmark suite centered on the workloads that dominate modern ML infrastructure: \textbf{LLM training} and \textbf{LLM inference}.
Each benchmark represents a concrete decision practitioners face (e.g., ``How long will fine-tuning Llama-70B take on 64 H100s with FSDP?'' or ``Can we serve Llama-7B at 50~QPS with P99 TTFT~$<$~500\,ms?'').
We then assess which tools can address each scenario.

\textbf{Training benchmarks} span four categories: (1)~\emph{kernel-level} prediction of transformer attention and MLP forward/backward pass latency across GPU types; (2)~\emph{communication-level} modeling of distributed training collectives (AllReduce for data parallelism, AllGather for tensor parallelism, All-to-All for expert parallelism) at 8--64 GPUs with hierarchical NVLink/InfiniBand topologies; (3)~\emph{system-level} training scaling, including memory capacity limits, activation checkpointing tradeoffs, and end-to-end training time estimation; and (4)~\emph{advanced training} scenarios including FP8 mixed-precision, sequence parallelism for long contexts, LoRA fine-tuning, and context window extension---optimizations that are standard practice but change performance characteristics by $2$--$4\times$.

\textbf{Inference benchmarks} cover five categories: (1)~\emph{single-request latency} separating prefill (compute-bound, quadratic in sequence length) from decode (memory-bandwidth-bound) phases at varying context lengths (128--8192 tokens); (2)~\emph{batched serving throughput} under realistic Poisson arrivals (QPS 2--50), testing continuous batching and chunked prefill scheduling; (3)~\emph{KV cache management}, predicting memory consumption as a function of sequence length and concurrency; (4)~\emph{architecture variants} including mixture-of-experts (MoE) routing and grouped-query attention (GQA); and (5)~\emph{production optimizations} including speculative decoding ($\sim$$3\times$ decode speedup), prefix caching for RAG workloads, disaggregated prefill/decode serving, and INT4 quantized inference---techniques deployed at scale by every major LLM provider.

Table~\ref{tab:llm-benchmark-coverage} shows which tools can address each benchmark category.
Experiments execute on Apple M2 Ultra (192\,GB RAM, Docker where available); deterministic tools verified bit-identical across three runs, stochastic tools report mean and P99 across fixed seeds.
All scripts and data are provided as supplementary material.

\begin{table}[t]
\centering
\caption{LLM benchmark coverage. \ding{51} = tool can produce predictions; ($\sim$) = partial coverage; --- = outside tool scope. 45 total scenarios across training (21) and inference (24).}
\label{tab:llm-benchmark-coverage}
\small
\begin{tabular}{lccccc}
\toprule
\textbf{Benchmark Category} & \rotatebox{70}{\textbf{NeuSight}} & \rotatebox{70}{\textbf{ASTRA-sim}} & \rotatebox{70}{\textbf{VIDUR}} & \rotatebox{70}{\textbf{Timeloop}} & \rotatebox{70}{\textbf{nn-Meter}} \\
\midrule
\multicolumn{6}{l}{\emph{LLM Training (21 scenarios)}} \\
T1: Training kernels (5) & ($\sim$) & --- & --- & --- & --- \\
T2: Distributed comm (6) & --- & 5/6 & --- & --- & --- \\
T3: Scaling \& memory (5) & --- & ($\sim$) & --- & --- & --- \\
T4: Advanced training (5) & --- & ($\sim$) & --- & --- & --- \\
\midrule
\multicolumn{6}{l}{\emph{LLM Inference (24 scenarios)}} \\
I1: Single-request latency (5) & ($\sim$) & --- & \ding{51} & --- & --- \\
I2: Batched serving (6) & --- & --- & \ding{51} & --- & --- \\
I3: KV cache \& memory (4) & --- & --- & ($\sim$) & --- & --- \\
I4: Architecture variants (4) & ($\sim$) & --- & ($\sim$) & --- & --- \\
I5: Production optimizations (5) & --- & --- & --- & --- & --- \\
\midrule
\textbf{Full coverage} & 0/45 & 5/45 & 12/45 & 0/45 & 0/45 \\
\textbf{Partial coverage} & 7/45 & 4/45 & 2/45 & 0/45 & 0/45 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key observation.} No single tool covers even a third of the LLM benchmark suite.
VIDUR provides the broadest LLM coverage (12/45 scenarios, all inference-side), while ASTRA-sim covers distributed communication (5/45, all training-side).
The advanced training (T4) and production inference optimization (I5) categories have \emph{zero} tool coverage---no evaluated tool models FP8 training, LoRA fine-tuning, speculative decoding, prefix caching, or quantized inference, despite these being standard practice.
Only 17/45 scenarios (38\%) have any tool coverage; \textbf{62\% of modern LLM workload scenarios are completely unaddressed}.
Sections~\ref{subsec:neusight-results}--\ref{subsec:nnmeter-results} present detailed per-tool results.
...
We evaluate five tools against our LLM-focused benchmark suite (Section~\ref{subsec:llm-benchmark-suite}), comparing independently measured results against published claims.
...
Our LLM-focused benchmark evaluation reveals four findings that would be invisible to a survey based solely on self-reported numbers:
...
\emph{Fourth}, \textbf{LLM workload coverage is critically sparse} (Table~\ref{tab:llm-benchmark-coverage}).
Against our 45-scenario LLM benchmark suite, the best tool (VIDUR) covers only 12 scenarios, all on the inference side.
Only 38\% of scenarios have any tool coverage (full or partial); the remaining 62\% are completely unaddressed.
Production-critical techniques---FP8 training, LoRA fine-tuning, speculative decoding, prefix caching, quantized inference---have zero tool coverage despite being standard practice.
End-to-end LLM training time prediction requires manual integration of at least three tools (NeuSight kernels $\rightarrow$ ASTRA-sim communication $\rightarrow$ custom memory model), with no existing framework to orchestrate them.
