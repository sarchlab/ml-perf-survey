...
\node[input] (wl) at (0,2.2) {Workload\\(ONNX/Chakra)}; \node[input] (hw) at (3,2.2) {Hardware\\config}; \node[input] (prof) at (6,2.2) {Profiling\\data};
\node[comp, fill=blue!15] (analytical) at (0,1.3) {Analytical\\Engine}; \node[comp, fill=red!15] (simulator) at (3,1.3) {Cycle-Accurate\\Simulator}; \node[comp, fill=purple!15] (mlmodel) at (6,1.3) {ML Cost\\Model};
\node[comp, fill=green!15, minimum width=3.2cm] (hybrid) at (3,0.5) {Hybrid Composition\\(analytical + learned)}; \node[comp, fill=orange!15, minimum width=6.5cm] (system) at (3,-0.2) {System Orchestrator (trace-driven)}; \node[input, draw=black!60, fill=gray!10] (output) at (3,-0.85) {Latency / Energy / Throughput};
...
\node[font=\tiny, text=blue!60, anchor=east] at (-0.5,1.3) {Timeloop, MAESTRO}; \node[font=\tiny, text=red!60, anchor=east] at (1.2,1.55) {Accel-Sim, GPGPU-Sim}; \node[font=\tiny, text=purple!60, anchor=west] at (7.6,1.3) {nn-Meter, TVM};
\node[font=\tiny, text=green!50!black, anchor=west] at (5,0.5) {NeuSight, Habitat}; \node[font=\tiny, text=orange!60!black, anchor=west] at (6.7,-0.2) {ASTRA-sim, VIDUR};
\end{tikzpicture}}
...
\node[level, fill=blue!15, draw=blue!50] (kernel) at (0,0) {Kernel / Operator}; \node[level, fill=green!15, draw=green!50] (model) at (0,0.8) {Model / End-to-End}; \node[level, fill=orange!15, draw=orange!50] (system) at (0,1.6) {System};
\draw[arrow] (kernel) -- (model); \draw[arrow] (model) -- (system);
\node[tool, anchor=east] at (-2.1,0) {NeuSight, nn-Meter,}; \node[tool, anchor=east] at (-2.1,-0.2) {TVM, GRANITE, TLP}; \node[tool, anchor=east] at (-2.1,0.8) {Paleo, Habitat,}; \node[tool, anchor=east] at (-2.1,0.6) {AMALI, Lumos, LIFE}; \node[tool, anchor=east] at (-2.1,1.6) {ASTRA-sim, VIDUR,}; \node[tool, anchor=east] at (-2.1,1.4) {SimAI, Frontier};
\node[err, anchor=west] at (2.1,0) {2--3\% error}; \node[err, anchor=west] at (2.1,0.8) {5--12\% error}; \node[err, anchor=west] at (2.1,1.6) {5--15\% error};
\draw[compos] (3.6,0.15) -- (3.6,1.45); \node[font=\scriptsize, text=red!50!black, rotate=90, anchor=south] at (3.9,0.8) {error accumulates};
\end{tikzpicture}}
...
\node[layer, fill=purple!12, draw=purple!40] (hw) at (0,0) {Layer 1: Hardware Design}; \node[tool, anchor=west] at (3.2,0) {Timeloop}; \node[layer, fill=blue!12, draw=blue!40] (kernel) at (0,0.8) {Layer 2: Kernel Prediction}; \node[tool, anchor=west] at (3.2,0.8) {NeuSight};
\node[layer, fill=red!8, draw=red!60, dashed, line width=1pt] (comp) at (0,1.6) {Layer 3: Model Composition}; \node[font=\tiny\bfseries, text=red!60!black, anchor=west] at (3.2,1.6) {CRITICAL GAP}; \node[layer, fill=orange!12, draw=orange!40] (dist) at (0,2.4) {Layer 4: Distributed Training}; \node[tool, anchor=west] at (3.2,2.4) {ASTRA-sim};
\node[layer, fill=green!12, draw=green!40] (serve) at (0,3.2) {Layer 5: Serving System}; \node[tool, anchor=west] at (3.2,3.2) {VIDUR};
...
\node[font=\tiny, text=black!50, anchor=east] at (-3.2,0) {5--10\% err}; \node[font=\tiny, text=black!50, anchor=east] at (-3.2,0.8) {2--9\% MAPE}; \node[font=\tiny, text=red!60!black, anchor=east] at (-3.2,1.6) {\textbf{5--15\% gap}}; \node[font=\tiny, text=black!50, anchor=east] at (-3.2,2.4) {5--15\% geo.}; \node[font=\tiny, text=black!50, anchor=east] at (-3.2,3.2) {$<$5\% err};
\draw[decorate, decoration={brace, amplitude=4pt, mirror}, thick, red!50] (-3.4,1.3) -- (-3.4,1.9); \node[font=\tiny, text=red!50!black, anchor=east] at (-3.8,1.6) {unmodeled};
\end{tikzpicture}}
...
\node[box, fill=yellow!20, dashed, draw=orange!60] (h1) at (0.75,0.55) {Launch\\overhead}; \node[box, fill=yellow!20, dashed, draw=orange!60] (h2) at (2.25,0.55) {Mem\\alloc}; \node[box, fill=yellow!20, dashed, draw=orange!60] (h3) at (3.75,0.55) {Data\\movement}; \node[box, fill=yellow!20, dashed, draw=orange!60] (h4) at (5.25,0.55) {Sync\\barrier};
\node[box, fill=green!10, minimum width=6.5cm] (model) at (3,1.2) {Model-level prediction}; \node[err] at (3,0.9) {5--12\% (hidden overhead accumulates)}; \node[font=\scriptsize\bfseries, anchor=east] at (-0.8,1.2) {Model};
\node[box, fill=orange!10, minimum width=6.5cm] (system) at (3,1.9) {System-level prediction}; \node[err] at (3,1.57) {+ communication, scheduling, contention}; \node[font=\scriptsize\bfseries, anchor=east] at (-0.8,1.9) {System}; \node[err] at (3,2.25) {5--15\% total error};
\draw[brace] (-0.5,-0.7) -- (6.5,-0.7) node[midway, below=4pt, font=\tiny, text=red!60!black] {$\sigma_{\text{model}} \approx \sigma_{\text{kernel}} \cdot \sqrt{N}$ (uncorrelated) to $N \cdot \sigma_{\text{kernel}}$ (correlated)};
\end{tikzpicture}}
