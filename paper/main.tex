...
We contribute an \textbf{accuracy-centered verification framework} replacing self-reported accuracy with reproducible evaluation---claimed error rates are overstated by $2$--$4\times$---via: (1)~a \textbf{28-scenario LLM benchmark suite} revealing 50\% of scenarios lack tool support; (2)~\textbf{independent verification} of five tools showing claims are overstated; (3)~a \textbf{unified pipeline} identifying the kernel-to-model composition gap; and (4)~a \textbf{coverage matrix} and research agenda for composition modeling, unified formats, and continuous validation.
\begin{figure}[t]
\centering
...
\node[input] (wl) at (0,2.5) {Workload\\(ONNX/Chakra)}; \node[input] (hw) at (3,2.5) {Hardware\\config}; \node[input] (prof) at (6,2.5) {Profiling\\data};
\node[comp, fill=blue!15] (analytical) at (0,1.5) {Analytical\\Engine}; \node[comp, fill=red!15] (simulator) at (3,1.5) {Cycle-Accurate\\Simulator}; \node[comp, fill=purple!15] (mlmodel) at (6,1.5) {ML Cost\\Model};
\node[comp, fill=green!15, minimum width=3.2cm] (hybrid) at (3,0.6) {Hybrid Composition\\(analytical + learned)};
\node[comp, fill=orange!15, minimum width=6.5cm] (system) at (3,-0.2) {System Orchestrator (trace-driven)};
\node[input, draw=black!60, fill=gray!10] (output) at (3,-0.9) {Latency / Energy / Throughput};
...
\node[font=\tiny, text=blue!60, anchor=east] at (-0.5,1.5) {Timeloop, MAESTRO};
\node[font=\tiny, text=red!60, anchor=east] at (1.2,1.75) {Accel-Sim, GPGPU-Sim};
\node[font=\tiny, text=purple!60, anchor=west] at (7.6,1.5) {nn-Meter, TVM};
\node[font=\tiny, text=green!50!black, anchor=west] at (5,0.6) {NeuSight, Habitat}; \node[font=\tiny, text=orange!60!black, anchor=west] at (6.7,-0.2) {ASTRA-sim, VIDUR};
\end{tikzpicture}%
}
...
\node[level, fill=blue!15, draw=blue!50] (kernel) at (0,0) {Kernel / Operator}; \node[level, fill=green!15, draw=green!50] (model) at (0,0.95) {Model / End-to-End}; \node[level, fill=orange!15, draw=orange!50] (system) at (0,1.9) {System};
\draw[arrow] (kernel) -- (model); \draw[arrow] (model) -- (system);
\node[tool, anchor=east] at (-2.1,0) {NeuSight, nn-Meter,}; \node[tool, anchor=east] at (-2.1,-0.2) {TVM, GRANITE, TLP}; \node[tool, anchor=east] at (-2.1,0.95) {Paleo, Habitat,}; \node[tool, anchor=east] at (-2.1,0.75) {AMALI, Lumos, LIFE}; \node[tool, anchor=east] at (-2.1,1.9) {ASTRA-sim, VIDUR,}; \node[tool, anchor=east] at (-2.1,1.7) {SimAI, Frontier};
\node[err, anchor=west] at (2.1,0) {2--3\% error}; \node[err, anchor=west] at (2.1,0.95) {5--12\% error}; \node[err, anchor=west] at (2.1,1.9) {5--15\% error};
\draw[compos] (3.6,0.15) -- (3.6,1.75); \node[font=\scriptsize, text=red!50!black, rotate=90, anchor=south] at (3.9,0.95) {error accumulates};
\end{tikzpicture}%
}
...
\textbf{DNN accelerators and GPUs.} Timeloop~\cite{timeloop2019} enumerates loop-nest mappings (5--10\%); MAESTRO~\cite{maestro2019} and Sparseloop~\cite{sparseloop2022} extend to data-centric and sparse tensors~\cite{diannao2014,eyeriss2016}; PIM tools~\cite{upimulator2024,attacc2024,neupims2024,paise2025} and PyTorchSim~\cite{pytorchsim2025}/ArchGym~\cite{archgym2023} represent newer approaches. Cycle-accurate simulators (GPGPU-Sim~\cite{gpgpusim2009}, Accel-Sim~\cite{accelsim2020}) achieve 0.90--0.97 IPC correlation at $10^3$--$10^4\times$ slowdown~\cite{dramsim3_2020,ramulator2_2023,dissectinggpu2025}; hybrid tools NeuSight~\cite{neusight2025} (2.3\%), Habitat~\cite{habitat2021} (11.8\%), AMALI~\cite{amali2025} (23.6\%), and TVM/Ansor/TLP~\cite{tvm2018,ansor2020,tlp2023,life2025,hermes2025,omniwise2025,swizzleperf2025,synperf2025,tenset2021} trade accuracy for speed.
\textbf{Distributed, serving, and edge.} ASTRA-sim~\cite{astrasim2023} replays Chakra traces~\cite{chakra2023} (5--15\%); SimAI~\cite{simai2025} (1.9\%), VIDUR~\cite{vidur2024}, Lumos~\cite{lumos2025} (3.3\%), and others~\cite{echo2024,paleo2017,distserve2024,frontier2025,podattention2025,aqua2025,throttllem2025,sailor2025} provide complementary coverage. nn-Meter~\cite{nnmeter2021} claims $<$1\% but is unverifiable (Section~\ref{sec:eval-results}); LitePred~\cite{litepred2024} (0.7\%) and HELP~\cite{help2021} (1.9\%) cover mobile platforms~\cite{esm2025,latencypredictorsnas2024}.
...
No single tool spans kernel execution through distributed training to serving SLAs (Table~\ref{tab:feature-matrix}); Figure~\ref{fig:pipeline-architecture} shows a five-layer pipeline: hardware design (Timeloop), kernel prediction (NeuSight), \textbf{model composition (CRITICAL GAP)}, distributed training (ASTRA-sim), and serving (VIDUR).
\begin{figure}[t]
\centering
\resizebox{\columnwidth}{!}{%
...
\node[layer, fill=blue!12, draw=blue!40] (kernel) at (0,0.95) {Layer 2: Kernel Prediction}; \node[tool, anchor=west] at (3.2,0.95) {NeuSight};
\node[layer, fill=red!8, draw=red!60, dashed, line width=1pt] (comp) at (0,1.9) {Layer 3: Model Composition}; \node[font=\tiny\bfseries, text=red!60!black, anchor=west] at (3.2,1.9) {CRITICAL GAP};
\node[layer, fill=orange!12, draw=orange!40] (dist) at (0,2.85) {Layer 4: Distributed Training}; \node[tool, anchor=west] at (3.2,2.85) {ASTRA-sim};
\node[layer, fill=green!12, draw=green!40] (serve) at (0,3.8) {Layer 5: Serving System}; \node[tool, anchor=west] at (3.2,3.8) {VIDUR};
...
\node[font=\tiny, text=black!50, anchor=east] at (-3.2,0) {5--10\% err}; \node[font=\tiny, text=black!50, anchor=east] at (-3.2,0.95) {2--9\% MAPE}; \node[font=\tiny, text=red!60!black, anchor=east] at (-3.2,1.9) {\textbf{5--15\% gap}};
\node[font=\tiny, text=black!50, anchor=east] at (-3.2,2.85) {5--15\% geo.}; \node[font=\tiny, text=black!50, anchor=east] at (-3.2,3.8) {$<$5\% err};
\draw[decorate, decoration={brace, amplitude=4pt, mirror}, thick, red!50] (-3.4,1.6) -- (-3.4,2.2); \node[font=\tiny, text=red!50!black, anchor=east] at (-3.8,1.9) {unmodeled};
\end{tikzpicture}%
}
...
\end{figure}
\textbf{Critical gap:} NeuSight's 5--9\% kernel MAPE grows to 10--28\% at model level via launch overhead, data movement, and synchronization---no validated composition model addresses this; the full pipeline also requires common workload formats and cross-hardware transfer (accuracy degrades $3$--$4\times$ outside training distributions).
...
\node[box, fill=yellow!20, dashed, draw=orange!60] (h1) at (0.75,0.6) {Launch\\overhead}; \node[box, fill=yellow!20, dashed, draw=orange!60] (h2) at (2.25,0.6) {Mem\\alloc}; \node[box, fill=yellow!20, dashed, draw=orange!60] (h3) at (3.75,0.6) {Data\\movement}; \node[box, fill=yellow!20, dashed, draw=orange!60] (h4) at (5.25,0.6) {Sync\\barrier};
\node[box, fill=green!10, minimum width=6.5cm] (model) at (3,1.35) {Model-level prediction}; \node[err] at (3,1.0) {5--12\% (hidden overhead accumulates)}; \node[font=\scriptsize\bfseries, anchor=east] at (-0.8,1.35) {Model};
\node[box, fill=orange!10, minimum width=6.5cm] (system) at (3,2.1) {System-level prediction}; \node[err] at (3,1.75) {+ communication, scheduling, contention}; \node[font=\scriptsize\bfseries, anchor=east] at (-0.8,2.1) {System}; \node[err] at (3,2.5) {5--15\% total error};
...
\end{figure}
\textbf{(3)~Hardware transfer and network fidelity:} Cross-family transfer (GPU$\rightarrow$TPU$\rightarrow$PIM~\cite{upimulator2024,attacc2024,neupims2024,paise2025}) remains unsolved; tools use analytical network abstractions that omit congestion~\cite{astrasim2023,triosim2025,ns3_2010}.
\textbf{(4)~Standardized evaluation:} No MLPerf~\cite{mlperf_training2020,mlperf_inference2020} equivalent exists; common benchmarks, portable formats (ONNX, Chakra~\cite{chakra2023}), and continuous validation~\cite{mlperfpower2025} are needed---nn-Meter's dependency rot~\cite{flashattention2022} exemplifies this gap.
