...
\textbf{(3)~Hardware transfer:} Cross-family transfer (GPU$\rightarrow$TPU$\rightarrow$PIM~\cite{upimulator2024,attacc2024,neupims2024,paise2025}) and congestion modeling~\cite{astrasim2023,triosim2025,ns3_2010} remain unsolved; domain adaptation techniques that fine-tune learned models on small target-hardware datasets offer a path forward but require architecture-aware embedding spaces. \textbf{(4)~Standardized evaluation:} No MLPerf~\cite{mlperf_training2020,mlperf_inference2020} equivalent exists for simulators; a community benchmark suite with portable formats~\cite{chakra2023}, reference measurements across GPU generations, and continuous validation~\cite{mlperfpower2025} is needed. \textbf{(5)~Reproducibility:} Dependency rot is a practical barrier---nn-Meter failed due to incompatible libraries; containerized environments and nightly CI pipelines that rebuild toolchains and regression-test against golden traces are needed.
